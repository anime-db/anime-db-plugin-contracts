...
`type` (например, `features` у `translation` или `locales` у `integration`),
а не любой ключ вне фиксированного списка. Причина: схема `manifest.json`
...
плагинов и реестру маркета, но не нужно ни хост-приложению, ни этому пакету,
поэтому заводить его в `Manifest` и релизить ради него контракт незачем —
манифест с ним просто проходит валидацию как есть.
...
валидации манифеста, причём причина будет выглядеть как поломка на стороне
плагинов, а не как следствие правки в контракте.
