...
`file_get_contents` **с URL** (вызов со статически известной локальной
строкой без схемы обёртки, например конфигом или шаблоном плагина, не
запрещён), любые функции `curl_*`, оператор обратных кавычек
(`` `command` ``), `eval`, а также динамические вызовы через
переменные-функции (`$fn()`, но не через array-callable вида
`[$this, 'method']`) и переменные-переменные (`$$name`).
