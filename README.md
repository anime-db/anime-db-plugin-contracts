...
несмотря на то что на момент добавления единственный потребитель правил
пакета анализирует исходники самого пакета контрактов, где плагинов нет:
`PluginType::Local` в манифестах этого пакета не встречается, поэтому
сейчас правило корректно ничего не делает на практике. Оно начнёт reально
работать, когда какой-то потребитель прогонит PHPStan с правилами этого
пакета по дереву исходников настоящего плагина — это отслеживается в
[anime-db/anime-db-plugins#111](https://github.com/anime-db/anime-db-plugins/issues/111).
Не включать правило до тех пор значило бы повторить ошибку, уже допущенную
в этом пакете однажды: два правила были описаны как «автоматический гейт»,
хотя ни разу не прогонялись на реальном коде плагинов. Честное решение —
прямо сказать про эту зависимость (как здесь), а не придерживать корректное
правило до готовности каждого потребителя его применять.
