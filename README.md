...
источником, объявляет `features` (плоский набор булевых флагов); код-плагин
`local`, реагирующий на события каталога и не ходящий в сеть (реализует
Symfony `EventSubscriberInterface`, не `ExternalIdResolutionInterface`), и
чисто декларативный ресурс переводов `translation` (набор файлов локалей,
без кода) `features` не объявляют.
...
`type` (например, `features` у `translation` или `local`), а не любой ключ
