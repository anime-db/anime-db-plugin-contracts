...
- [Манифест плагина (`manifest.json`)](#манифест-плагина-manifestjson)
  - [`ui` — стили и скрипты плагина](#ui--стили-и-скрипты-плагина)
  - [Незнакомые поля манифеста](#незнакомые-поля-манифеста)
...
Если виджету пока нечего показать, `render()` возвращает HTML с префиксом
`WidgetPendingUpdate::MARKER` (или через `WidgetPendingUpdate::mark($html)`):
хост вырежет маркер и отдаст этот ответ без `max-age`. Маркер должен стоять
ровно в начале строки — без пробелов, переводов строки и BOM (префикс
сравнивается побайтно). Кроме кеширования маркер ничего не меняет; хост
сам виджет не перезапрашивает — обновление при необходимости виджет
обеспечивает сам.
...
Диспатчится **после** того, как изменение уже закоммичено, и подписчик
...
инлайн, отработает синхронно внутри того же запроса или процесса, что
породил событие.
...
Ожидаемая форма `locale` — первичный сабтег ISO 639-1 в нижнем регистре
(`ja`, `ru`, `en`, `ko`); `null` — источник не объявил язык (например, для
нетипизированного списка синонимов), это штатное значение, а не признак
ошибки. Хост приводит значение к нижнему регистру, срезает региональный
сабтег (`ru-RU` → `ru`) и нераспознанное значение обращает в `null` —
поэтому исходные обозначения языка у источника (`russian`, `Japanese` и
т. п.) в это поле передавать нельзя, их обязан нормализовать сам плагин.
...
- путь начинается с `assets/` — это единственный каталог плагина, который
  хост подключает сам;
- путь относительный: без ведущего `/`, без `\`, `:` и сегмента `..`;
- расширение соответствует списку в нижнем регистре: `.css` для `css`,
  `.js` для `js` (`assets/style.CSS` отклоняется);
- без повторов внутри одного списка.
...
#### Область действия CSS плагина

Хост подключает `ui.css` в тот же документ, где живут собственные стили
приложения. Отдельного документа, shadow DOM или iframe у UI плагина нет,
поэтому **каскад глобален: селектор плагина применяется ко всей странице**, а
не только к его виджету или странице настроек.

Отсюда соглашение: **все селекторы плагина вкладываются в его корневой
класс.**

```css
/* Перекрасит заголовки во всём приложении, включая его собственные экраны. */
h2 { color: #c00; }

/* Действует только внутри разметки плагина. */
.animedb-shikimori-carousel h2 { color: #c00; }
```

Корневой класс плагин задаёт сам в своей разметке; вести его от идентификатора
плагина (`animedb-shikimori` → `.animedb-shikimori-carousel`) — простой способ
не столкнуться с другим плагином.

Это **соглашение, а не проверяемое правило**. Валидатора CSS нет и не
планируется: технической изоляции без shadow DOM или iframe не существует, а
разбор чужого CSS на «правильную вложенность» — хрупкая конструкция, которую
легко обойти и дорого поддерживать. Соблюдение проверяется на ревью и остаётся
на ответственности автора плагина.

Симметричная оговорка про `ui.js`: скрипт плагина исполняется в том же
документе и имеет полный доступ к DOM. Ограничить это технически тем более
нечем, поэтому действует то же правило — работать со своей разметкой, а не с
чужой.

Практическое следствие для обеих сторон: хост не обещает стабильность имён
своих классов. Если плагин цепляется к разметке приложения, он берёт на себя
риск, что она изменится в следующей версии.
...
- запуск процессов и сырых сетевых сокетов — всегда, независимо от
  ...
  `stream_socket_client`, `stream_socket_server`, `dns_get_record`,
...
- создание известных сетевых клиентов и процесс-лаунчеров через `new` —
  ...
  `Symfony\Component\Process\Process`, а также любой класс из **дерева
  зависимостей хост-приложения**, реализующий
  ...
  инжектирует хост). Класс, объявленный в самом анализируемом коде плагина
  (например, собственный декоратор плагина поверх инжектированного хостом
  PSR-18 клиента — троттлинг, ретраи, логирование), под этот пункт не
  попадает: он не зависимость хост-приложения, а реализация ровно той
  абстракции, к которой отсылает сообщение об ошибке; что он делает
  внутри — ловят остальные проверки правила. Проверка по типу, а не по
  буквальному имени класса — подкласс запрещённого класса
  (`class MySoap extends SoapClient {}`) пойман так же, как и сам класс;
- статический вызов известных фабрик сетевых клиентов — всегда,
  ...
  сама;
- чтение/включение/**запись** файла со статически известным URL (схема
  обёртки вида `https://`, `ftp://` и т.п. в первом аргументе, включая
  случай, когда URL — литеральный префикс строковой конкатенации с
  динамическим хвостом, например `'https://api.example.com/'.$id`):
  `file_get_contents`, `fopen`, `copy`, `file`, `readfile`,
  `simplexml_load_file`, `getimagesize`, `file_put_contents`,
  `include`/`include_once`/`require`/`require_once`, а также
  `DOMDocument::load()`/`DOMDocument::loadHTMLFile()`. Вызов с локальным
  путём (например, конфигом или шаблоном плагина) не запрещён — правило
  целится в скрытый сетевой запрос под видом файловой операции, а не в
  файловый ввод-вывод как таковой. Полностью динамический URL (например,
  значение, прочитанное из свойства без литерального префикса схемы)
  правило не ловит: у него нет статически выводимого признака,
  отличающего URL от локального пути, а эвристика по имени
  переменной/свойства ненадёжна в обе стороны;
...
Правило — денилист, а не аллоулист: он перечисляет конкретные функции и
классы, а не ограничивает плагин набором разрешённых пространств имён.
Денилист по построению отстаёт от дерева зависимостей хост-приложения —
новая зависимость хоста, дающая доступ к сетевому/процессному/файловому
примитиву, остаётся потенциальным обходом, пока список не расширен под
неё. Автоматики, которая обнаруживала бы такой дрейф, нет: при добавлении
или обновлении зависимости хост-приложения её нужно вручную проверить на
такие примитивы и, если найдены, добавить в соответствующий список выше.
...
- **манифеста нет вообще нигде выше по дереву** — правило молчит для этого
  файла. Отсутствие манифеста — забота `ManifestValidator`, а не этого
  правила; это же ожидаемое состояние при анализе исходников самого пакета
  контрактов (в нём манифеста плагина нет и не будет);
- **манифест найден, но ему нельзя доверять** — не читается, не является
  валидным JSON, не проходит проверки `ManifestValidator`, не содержит поле
  `type`, либо значение `type` не распознано установленной версией пакета
  (например, тип, добавленный более новой версией контракта, чем та, что
  анализирует код) — в этом случае правило **всегда** сообщает об ошибке,
  независимо от того, каким мог бы оказаться тип. Молчание здесь означало
  бы, что плагин прошёл проверку по причине «не смогли понять, какой он», а
  не «он не local».

Сетевые типы, которые правило запрещает для `local`:

- `Psr\Http\Client\ClientInterface` — PSR-18 транспорт: отправка запроса
  через него и есть сетевой вызов;
- `Psr\Http\Message\RequestFactoryInterface` — строит объект запроса,
  единственное практическое назначение которого — быть переданным в
  `ClientInterface`. `Psr\Http\Message\StreamFactoryInterface` (третья
  PSR-17 фабрика, от которой зависит `OAuth\AbstractOAuthClient`) в список
  намеренно не входит — она строит поток в памяти, не более сетевая, чем
  локальные данные, никакого сетевого смысла сама по себе не несёт;
- `SearchByPluginInterface` — `find()` по определению ищет/сопоставляет
  тайтл с внешним источником;
- `FillerInterface` (расширяет `SearchByPluginInterface`) — `findById()`
  тоже обращается к внешнему источнику за деталями; перечислен в списке
  явно, вместе с расширяемым интерфейсом, чтобы решение по каждой роли было
...
- `DownloadServiceInterface` — `enqueue()` запускает скачивание с внешнего
  ...
  автора задачи «заведомо сетевого» интерфейса.

Интерфейсы контракта, для которых явно принято обратное решение — «не
сетевой»:

- `ExternalIdResolutionInterface` — `resolveExternalId()` только сопоставляет
  уже переданный список ссылок, сам вызов сети не делает (интерфейсы выше,
  которые его расширяют, остаются сетевыми за счёт своих *других* методов,
  поэтому перечислены отдельно, а не через наследование от этого интерфейса);
- `CatalogWidgetInterface`/`EntryWidgetInterface` — `render()` строит
  разметку из того, что виджету уже доступно; ничего в интерфейсе не требует
  внешнего источника (виджету статуса закачки или локальных рекомендаций он
  не нужен вовсе);
- `SettingsPageInterface` — `render()` строит разметку формы настроек;
  интерактивные шаги, которые могут обращаться к вендору (старт OAuth, обмен
  токена), идут через собственные роуты плагина, не через этот интерфейс;
- `CatalogReaderInterface` — read-only проекция уже смёрженного локального
  состояния каталога — собственный пример автора задачи «заведомо не
  сетевого» интерфейса;
