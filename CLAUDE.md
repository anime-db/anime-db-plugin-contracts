...
- **`Background\BackgroundTaskQueueInterface`/`BackgroundTaskHandlerInterface`
  ...
  в обработчике не существует. `BackgroundTask::$name` — id плагина, чтобы
  различать свои задачи; ядро его не интерпретирует. `BackgroundTask::$payload`
...
- **`Catalog\AnimeFilesChangedEvent`/`FilesChangeReason`.** Одно событие на
  ...
  того, как изменение уже сохранено; подписчик вызывается синхронно.
